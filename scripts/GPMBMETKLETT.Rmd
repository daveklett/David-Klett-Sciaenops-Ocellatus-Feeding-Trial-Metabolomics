...
#1. Plot weight vs total length and get a regression line to see if any diet grew weird; also can see if we can predict the growth of the red drum in this study
...
#2. log transform data for length weight relationship and do a new regression
...
#3. Exponential Model; wanted to see if there was a better model for weight and total length
...
#4. Plot total length and liver weight and see if differences occur
...
#4.1. Taking out diet 8 makes model much better; shows difference in pellets w/natural diets
...
#5. Plot weight and liver weight (HSI) and see if differences occur
...
#5.2.Taking out diet 8 makes model much better; shows difference in pellets w/natural diets
...
#1. Plot weights from Justin weighing individual tanks (n = ~ 44-48 fish per diet)
...
#2. Plot data taken during dissection sampling (n = 9 per diet)
...
#3. Plot data from all fish taken during dissection and for drying (n = 19 per diet)
...
#4. Plot FCR data for each diet (n = 3 tanks per diet)
