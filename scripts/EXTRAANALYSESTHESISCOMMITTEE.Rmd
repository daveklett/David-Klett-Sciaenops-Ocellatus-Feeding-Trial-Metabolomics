...
boxplot(lit$`protein intake fish` ~ lit$diet, xlab = "Diet", ylab = "Protein Intake per Fish (Dry Grams)", main = "Average Protein Intake per Fish Across Diets", las = 1,par(cex.axis=0.70)) # Make plot
dev.off()

#1. Protein Efficiency
tiff("Protein Efficiency.tiff", width = 6, height = 5, units = 'in', res = 300)
boxplot(lit$`protein effeciency ratio` ~ lit$diet, xlab = "Diet", ylab = "Protein Efficiency (Dry Wt Gain (g) / Dry Protein Fed (g)", main = "Average Protein Efficiency Across Diets", las = 1,par(cex.axis=0.70)) # Make plot
dev.off()
